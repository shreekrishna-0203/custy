...
interface TranscriptE{
  text: string;
  participantId: string;
  timestamp: Date;
}

interface TranscriptM{
  type: 'transcript';
  transcript: TranscriptE;
}
...
export default function MeetingRoom() {
  ...
  const [peerId, sPeerId] = useState<string>('');
  const [remotePeerId, sRemotePeerId] = useState<string>('');
  const [isAudioEnabled, sIsAudioEnabled] = useState(true);
  const [isVideoEnabled, sIsVideoEnabled] = useState(true);
  const [isConnected, sIsConnected] = useState(false);
  const [remoteTranscripts, sRemoteTranscripts] = useState<TranscriptE[]>([]);
  const [currentLocalTranscript, sCurrentLocalTranscript] = useState<string | null>(null);
  const [currentRemoteTranscript, sCurrentRemoteTranscript] = useState<string | null>(null);
  const [copied, sCopied] = useState(false);
  const transcriptTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  const [user, sUser] = useState<User | null>(null);
  const [showSubtitles, sShowSubtitles] = useState(false);
  const [summary, sSummary] = useState('');
  const [summarizing, sSummarizing] = useState(false);
  const [localTranscripts, sLocalTranscripts] = useState<TranscriptE[]>([]);
  const [selectedLanguage, sSelectedLanguage] = useState<Lang>(LANGUAGES[0]);
  const [showLanguageMenu, sShowLanguageMenu] = useState(false);
  const [isListening, sIsListening] = useState(false);
  const [transcript, sTranscript] = useState('');
  ...
  const setupDataChannel = (conn: DataConnection) => {
    ...
    conn.on('data', (data: unknown) => {
      ...
      try {
        const message = data as TranscriptM;
        if (message.type === 'transcript') {
          ...
          sCurrentRemoteTranscript(receivedTranscript.text);
          if (transcriptTimeoutRef.current) {
            clearTimeout(transcriptTimeoutRef.current);
          }
          transcriptTimeoutRef.current = setTimeout(() => {
            sCurrentRemoteTranscript(null);
          }, 3000);

          sRemoteTranscripts(prev => {
  ...
  const sendTranscript = (transcript: TranscriptE) => {
    console.log('Attempting to send transcript:', transcript);
    sCurrentLocalTranscript(transcript.text);
    sLocalTranscripts((prev: TranscriptE[]) => {
  ...
  const toggleAudio = () => {
    if (streamRef.current) {
      ...
      sIsAudioEnabled(!isAudioEnabled);
    }
  };
  ...
  const toggleVideo = () => {
    if (streamRef.current) {
      ...
      sIsVideoEnabled(!isVideoEnabled);
    }
  };

  const toggleSubtitles = () => {
    sShowSubtitles(!showSubtitles);
  };
  ...
  const handleSummarizeMeeting = async () => {
    ...
    try {
      ...
      if (!allText.trim()) {
        sSummary('No subtitles recorded to summarize.');
        return;
      }
      ...
      if (Array.isArray(result) && result.length > 0) {
        sSummary(result[0].summary_text || 'Could not generate a summary.');
      } else {
        sSummary('Could not generate a summary. Please try again.');
      }
    } catch (error) {
      console.error('Error summarizing meeting:', error);
      if (error instanceof Error) {
        sSummary(`Error summarizing meeting: ${error.message}. Please check your API key and network connection.`);
      } else {
        sSummary('Error summarizing meeting. Please try again.');
      }
    } finally {
      sSummarizing(false);
    }
  ...
  const initializeRecognition = () => {
    ...
    const recognition = new (window as { webkitSpeechRecognition: new () 
      => SR }).webkitSpeechRecognition();
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (event.results[event.results.length - 1].isFinal) {
        const newTranscript: TranscriptE = {
          text: transcript,
  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-900 to-black text-white overflow-hidden">
      ...
      <header className="backdrop-blur-xl bg-black/30 border-b
       border-white/10 px-6 py-4 relative z-10 flex-shrink-0">
        <div className="flex justify-between items-center 
        max-w-7xl mx-auto w-full">
          <div className="flex items-center space-x-6">
            <Link to="/" className="flex items-center space-x-3">
              <div className="p-2 bg-gradient-to-r from-blue-500
          ...
          <div className="flex items-center space-x-4">
            ...
            {user ? (
              ...
              <Link
                to="/login"
                className="px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500
                 hover:to-purple-500 rounded-xl 
      ...
      <main className="flex-1 relative overflow-auto p-6 max-h-[calc(100vh-64px)]">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/5
         to-purple-600/5 rounded-3xl pointer-events-none"></div>
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-blue-500/10 
        ...
        <div className="max-w-7xl mx-auto relative z-10 min-h-full flex flex-col">
                ...
                <div className="bg-gray-800/50 backdrop-blur-sm p-4 rounded-xl font-mono text-sm text-center border border-white/5">
                  {peerId || (
                    <div className="animate-pulse">
                      <div className="h-4 bg-gray-600 
                      rounded w-32 mx-auto"></div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                ...
                <div className="flex space-x-3">
                  <input
                    ...
                    value={remotePeerId}
                    onChange={(e) => sRemotePeerId(e.target.value)}
                  />
                  <button
                    onClick={callPeer}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 flex-grow">
            <div className="relative group">
              ...
              <div className="relative overflow-hidden rounded-3xl
               bg-gradient-to-br from-gray-800/50
                to-gray-900/50 backdrop-blur-sm border
                ...
                {!isVideoEnabled && (
                  <div className="absolute inset-0 bg-gradient-to-br
                   from-gray-800/90 to-gray-900/90 rounded-3xl 
          ...
          {transcript && (
            ...
            z-10 max-w-3xl mx-auto bg-gray-900/80
          ...
          <div className="flex justify-center mt-auto">
            <div className="flex items-center space-x-4 bg-black/40 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 shadow-2xl">
              <div className="relative">
                <button
                  onClick={() => sShowLanguageMenu(!showLanguageMenu)}
